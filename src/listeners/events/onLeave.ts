...
export default class OnLeaveListener extends BushListener {
	...
	public async exec(member: GuildMember): Promise<void> {
		if (this.client.config.environment !== 'development') {
			...
			if (welcomeChannel) {
				let success = true;
				const welcome = <TextChannel>(
					this.client.channels.cache.get(welcomeChannel)
				);
				if (member.guild.id != welcome.guild.id)
					throw 'Welcome channel must be in the guild.';
			...
			if (member.guild.id == '516977525906341928') {
				...
				if (roles) {
					...
					if (ExistingData.length != 0) {
						const Query = await stickyRoleDataSchema.findByIdAndUpdate(
							ExistingData[0]['_id'],
							{
								id: member.id,
								left: Date.now(),
						...
						await Query.save().then(() => {
							if (this.client.config.info) {
								log.info(
									'RoleData',
									`Updated info for <<${member.user.tag}>>.`
								);
							}
						});
					} else {
						...
						await roles.save().then(() => {
							if (this.client.config.info) {
								log.info(
									'RoleData',
									`Created info for <<${member.user.tag}>>.`
								);
							}
						});
					}
