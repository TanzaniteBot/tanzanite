...
export default class CommandMissingPermissionsListener extends BushListener {
	...
	public async exec(message: Message, command: BushCommand | null | undefined, type: 'client' | 'user', missing: unknown): Promise<void> {
		...
		if (type == 'client') {
			...
			await message.reply(`<:no:787549684196704257> I am missing the \`${missing}\` permission(s) required for the \`${message.util.parsed.command}\` command.`).catch(() => {});
		} else if (type == 'user') {
			// eslint-disable-next-line @typescript-eslint/no-empty-function
			await message.reply(`<:no:787549684196704257> You are missing the \`${missing}\` permission(s) required for the \`${message.util.parsed.command}\` command.`).catch(() => {});
		}
	}
}
