...
export default class BlacklistedFileListener extends BushListener {
	...
	public async exec(message: Message): Promise<void> {
		...
		const embedAttachments = message.embeds.filter(e =>
		...
		for (const attachment of message.attachments) {
			try {
				...
				const blacklistData = this.blacklistedFiles.find(h =>
					h.hash.some(h => h === hash)
				);
			...
			try {
				...
				await this.client.log({
					embed: {
						...
						description: stripIndent`
							Blacklisted ${foundFiles.length === 1 ? 'file was' : 'files were'} deleted in ${
							message.channel
						}.
				...
				if (this.client.config.info) {
					...
					log.info(
						...
						`Deleted <<${foundFiles
							.map(f => f.description)
							.join(' and ')}>> sent by <<${message.author.tag}>> in ${
				...
				await this.client.log({
					embed: {
						...
						description: stripIndent`
							Blacklisted ${foundFiles.length === 1 ? 'file was' : 'files were'} sent in ${
							message.channel
						}.
							Author: <@!${message.author.id}> (${message.author.tag})
							Files found: ${foundFiles
							...
							Unable to delete file, an error occurred. (${await this.client.consts.haste(
								e.stack
							)})
						`,
