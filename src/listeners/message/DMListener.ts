	...
	public async exec(message: Message): Promise<void> {
		try {
			if (message.channel.type === 'dm') {
				...
				if (
					message.attachments.filter(a => typeof a.size == 'number').size == 1
				) {
					dmlogembed.setImage(
						message.attachments.filter(a => typeof a.size == 'number').first()
							.proxyURL
					);
				...
				const dmChannelid = await db.globalGet(
					'dmChannel',
					'783129374551572512'
				);
				const dmchannel = <TextChannel>(
					this.client.channels.cache.get(dmChannelid as string)
				);
