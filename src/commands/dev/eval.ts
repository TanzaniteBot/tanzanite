	...
	public async exec(
		...
		if (bad_phrases.some(p => code.includes(p)) && !sudo) {
			await message.util.send(
				'This eval was blocked by smooth brain protection™.'
			);
			return;
		}
		...
		try {
			...
			if (typeof output !== 'string')
				output = inspect(output, { depth: selDepth });
			...
			output = output.replace(
				new RegExp(
					[...this.client.credentials.devToken].reverse().join(''),
					'g'
				),
				'[Dev Token Omitted]'
			);
			output = output.replace(
				new RegExp(
					this.client.credentials.MongoDB.toString().replace(
						/[.*+?^${}()|[\]\\]/g,
						'\\$&'
					),
					'g'
			...
			output = output.replace(
				new RegExp(
					[...this.client.credentials.hypixelApiKey.toString()]
						.reverse()
						.join(''),
					'g'
				),
				...
				.addField(
					...
					code.length > 1012
						? 'Too large to display. Hastebin: ' +
								(await this.client.consts.haste(code))
						: '```js\n' + code + '```'
				)
				.addField(
					'📤 Output',
					output.length > 1012
						? 'Too large to display. Hastebin: ' +
								(await this.client.consts.haste(output))
						: '```js\n' + output + '```'
				)
				.setColor('#66FF00')
				.setFooter(
			...
			embed
				...
				.addField(
					...
					e.length > 1012
						? 'Too large to display. Hastebin: ' +
								(await this.client.consts.haste(e))
						: '```js\n' +
								e +
								'```Full stack:' +
