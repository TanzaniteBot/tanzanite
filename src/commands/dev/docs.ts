...
const format = (txt: string): string =>
	txt.replace(/<.+>/g, '```\n').replace(/<\/.+>/g, '```');
...
export default class SayCommand extends BushCommand {
	public constructor() {
		super('docs', {
			...
			description: {
				content:
					'A command to find things in the docs for discord.js, and discord-akairo soon™',
				usage: 'docs',
				...
				if (c === null) {
					...
					if (c.props === undefined || c.methods === undefined) {
						const embed = new MessageEmbed()
							...
							.setDescription(
								`${format(
									c.description
								)}\n\n[Docs link](http://discord.js.org/#/docs/main/stable/class/${
									c.name
								})`
							)
							.setFooter(
					...
					return message.util.reply(embed).catch(() => {
						...
						const embedSlim = new MessageEmbed()
							...
							.addField('| Methods', meths, true)
							.setFooter(
								'This response was minified to get around the discord character limit'
							)
							.setColor(this.client.consts.DefaultColor);
