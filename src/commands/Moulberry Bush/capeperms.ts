...
export default class CapePermCommand extends BushCommand {
	...
	public async exec(message: Message, { user }: { user: string }): Promise<Message> {
		...
		try {
			...
			return message.util.reply(`<:error:837123021016924261> \`${user}\` doesn't appear to be a valid username.`);
		}
		...
		if (capeperms == null) {
			return message.util.reply(`<:error:837123021016924261> There was an error finding cape perms for \`${user}\`.`);
		} else {
			if (capeperms?.perms) {
				let index = null;
