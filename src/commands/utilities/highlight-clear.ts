...
export default class HighlightClearCommand extends BotCommand {
	...
	public override async exec(message: CommandMessage | SlashMessage) {
		...
		if (!confirm) {
			return await message.util.reply(`${emojis.warn} You decided not to clear your highlight list.`);
		}

		const success = await this.client.highlightManager.removeAllHighlights(message.guild.id, message.author.id);

		if (!success) {
			return await message.util.reply(`${emojis.error} There was an error clearing your highlight list.`);
		}
