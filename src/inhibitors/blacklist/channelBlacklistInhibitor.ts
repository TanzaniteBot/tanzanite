...
export default class ChannelBlacklistInhibitor extends BotInhibitor {
	...
	public exec(message: Message): boolean {
		if ((this.client.ownerID.includes(message.author.id) != true) ||(this.client.config.superUsers.includes(message.author.id) != true)|| (message.member.roles.cache.some(r => this.client.config.roleWhitelist.includes(r.id)) != true)) {
